...
Strategy.prototype.authenticate = function (req, options) {
  ...
  function validated(err, ok, ctx) {
    ...
    // RP ID must be a registrable domain suffix or equal to current effectiveDomain
    const effectiveDomain = utils.getEffectiveDomain(req);
    const rpId = utils.getRPID(req);

    if (!effectiveDomain.endsWith(rpId)) {
      return self.fail(
        {
          message:
            "RP ID is not a valid suffix for this host's effective domain",
        },
        403,
      );
    }
