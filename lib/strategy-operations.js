...
function validateCredentialsGet(self, req, ctx) {
  ...
  try {
    let params = [id, userHandle, flags];
    ...
    const paramsToPass = [...params.slice(0, arity - 1), done];

    return self._verify(...paramsToPass);
  } catch (ex) {
...
function validateCredentialsCreate(self, req, ctx) {
  ...
  try {
    ...
    let arity = self._register.length;
    ...
    return self._register(...paramsToPass);
  } catch (ex) {
    return self.error(ex);
  }
}
