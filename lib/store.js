...
SessionStore.prototype.verify = function (req, challenge, cb) {
  ...
  process.nextTick(function () {
    const info = req.session[self._key];
    delete req.session[self._key];

    if (!info) {
      return cb(null, false, {
        message: "Unable to verify authentication challenge.",
      });
    }
    if (!info.challenge) {
      return cb(null, false, {
    ...
    const issuedChallenge = base64url.toBuffer(info.challenge);
